...
static int smt_respond(fwk_id_t channel_id, const void *payload, size_t size)
{
	...
	if (memory == (void *)0xfffff000) {
		memory = (void *)phys_to_virt((paddr_t)memory, MEM_AREA_IO_NSEC);
	}
...
static struct mod_smt_memory * smt_get_memory(fwk_id_t channel_id)
{
	...
	{
		void *memory = (struct mod_smt_memory*)
...
static const struct mod_smt_driver_input_api driver_input_api = {
	...
	.get_memory = smt_get_memory,
};

/*
 * Framework API
 */
static int hmbox_init(fwk_id_t module_id, unsigned int element_count,
                    const void *data)
{
...
static int hmbox_channel_init(fwk_id_t channel_id, unsigned int unused,
                            const void *data)
{
    struct smt_channel_ctx *channel_ctx;
    ...
    if ((channel_ctx->config->mailbox_address == 0) ||
...
static int hmbox_start(fwk_id_t id)
{
    return FWK_SUCCESS;
}

const struct fwk_module module_hmbx = {
    .name = "host mailbox",
    .type = FWK_MODULE_TYPE_SERVICE,
    .api_count = MOD_HOST_MBX_API_IDX_COUNT,
    .init = hmbox_init,
    .element_init = hmbox_channel_init,
    .bind = smt_bind,
    .start = hmbox_start,
