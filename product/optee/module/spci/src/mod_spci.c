...
void scmi_server_raise_event(uint32_t __unused id)
{
    ...
    /*
     * Mailbox ID not considered for now, assume OSPM0 mailbox.
     * Check if mailbox notification API carries mailbox identifier.
     */
...
static const struct fwk_element spci_element_table[] = {
    /* FIXME: remove the element if no secure client expected */
    [SPCI_DEVICE_IDX_S] = {
        .name = "SPCI TEE",
        .sub_element_count = 1,
