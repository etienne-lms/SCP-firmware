...
{
    struct stm32_reset_dev_ctx *ctx;
    ...
    FMSG("SCMI reset %lu: assert", ctx->reset_id);
...
static int reset_deassert(fwk_id_t dev_id)
{
    ...
    FMSG("SCMI reset %lu: deassert", ctx->reset_id);
                                    ...
                                    const void *data)
{
    ...
    const struct mod_stm32_reset_dev_config *dev_config = data;
