...
struct scmi_reset_ctx {
    ...
    const struct mod_scmi_reset_config *config;
...
static unsigned int count_domains(void)
{
    unsigned int i = 0;
    unsigned int count = 0;

    for (i = 0; i < scmi_reset_ctx.config->agent_count; i++)
	count += scmi_reset_ctx.config->agent_table[i].device_count;
                                       ...
                                       const uint32_t *payload)
{
    ...
    outmsg.attributes = count_domains();
 ...
 * \c mod_scmi_reset_agent structure within the agent table.
 */
static int get_agent_entry(fwk_id_t service_id,
			   const struct mod_scmi_reset_agent **agent)
{
    int status = 0;
    unsigned int agent_id = 0;

    status = scmi_reset_ctx.scmi_api->get_agent_id(service_id, &agent_id);
    if (status != FWK_SUCCESS)
        return status;

    if (agent_id >= scmi_reset_ctx.config->agent_count)
        return FWK_E_PARAM;

    *agent = &scmi_reset_ctx.agent_table[agent_id];

    return FWK_SUCCESS;
}

static int get_reset_device(fwk_id_t service_id, unsigned int domain_id,
                          const struct mod_scmi_reset_device **reset_device)
{
    int status = 0;
    const struct mod_scmi_reset_agent *agent_entry = NULL;
    ...
    *reset_device = &agent_entry->device_table[domain_id];

    assert(fwk_module_is_valid_element_id((*reset_device)->element_id));
    if (!fwk_module_is_valid_element_id((*reset_device)->element_id))
	    return FWK_E_PANIC;
 ...
 */
static bool service_is_permitted(const struct mod_scmi_reset_device *device,
                                enum mod_scmi_reset_permissions request)
{
    return device->permissions & request;
}

static int reset_attributes_handler(fwk_id_t service_id,
                                    const uint32_t *payload)
{
    const struct mod_scmi_reset_device *reset_device = NULL;
    struct scmi_reset_attributes_a2p params = { 0 };
    ...
    if (!service_is_permitted(reset_device, MOD_SCMI_RESET_PERM_ATTRIBUTES)) {
        outmsg.status = SCMI_DENIED;
        goto exit;
    }
...
{
    int status = FWK_SUCCESS;
    struct scmi_reset_request_a2p params = { 0 };
    ...
    if (!service_is_permitted(reset_device, MOD_SCMI_RESET_PERM_RESET)) {
        outmsg.status = SCMI_DENIED;		/* Should be obfuscated? */
        goto exit;
    }
    ...
    if (assert_reset) {
	status = reset_api->reset_assert(reset_device->element_id,
					 params.reset_state);
        if (status != FWK_SUCCESS) {
            outmsg.status = SCMI_NOT_SUPPORTED;
            goto exit;
    ...
    if (deassert_reset) {
	status = reset_api->reset_deassert(reset_device->element_id);
        if (status != FWK_SUCCESS) {
            outmsg.status = SCMI_NOT_SUPPORTED;
            goto exit;
        }
...
static int scmi_reset_init(fwk_id_t module_id, unsigned int element_count,
                           const void *data)
{
    const struct mod_scmi_reset_config *config = NULL;

    config = (const struct mod_scmi_reset_config *)data;

    if ((config == NULL) || (config->agent_table == NULL))
        return FWK_E_PARAM;

    scmi_reset_ctx.config = config;
    scmi_reset_ctx.agent_table = config->agent_table;
...
static int scmi_reset_bind(fwk_id_t id, unsigned int round)
{
    ...
    return fwk_module_bind(FWK_ID_MODULE(FWK_MODULE_IDX_RESET),
			   FWK_ID_API(FWK_MODULE_IDX_RESET, 0),
			   &scmi_reset_ctx.reset_api);
}
